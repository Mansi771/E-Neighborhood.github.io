...
<!--PAge for  viewing Buyer and activating deactivating it  -->
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body"><br>
                               <center> <h4 class="card-title">List of Buyer</h4></center>
                        ...
                        <table width="100%" class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline collapsed" id="dataTables-example" role="grid" aria-describedby="dataTables-example_info" style="width: 100%;">
                                <thead>
                                    <tr role="row">
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 83px;">Buyer Name</th>
...
////here to __ For displaying login approval list with similar area for adminand seller//////////////////
                               ...
                               $user=$_SESSION["email"];
                                                ...
                                                $ar=$row1["area"];
                                                    ...
                                                    $qry="select * from registration where isSeller='no' AND reg_app='no' and r_area='$ar' and user_email_status='verified'";
//////////////////////////// //here//////////////////////////////////////
                                ...
                                <tbody>
                                    <tr class="gradeA odd" role="row">
                                        ...
                                        <td><a href="view_profile_Buyer_aa.php?id=<?php echo $row['us_id'];?>  &email=<?php echo $row['e_mail']; ?>"><?php echo $row['uname']; ?></a></td>
                                        <td><?php echo $row['r_area']; ?></td>
                                        <td><?php echo $row['e_mail']; ?></td>
                                        <td><?php echo $row['register_date']; ?></td>
                                        <td><?php echo $row['active']; ?></td>
                                         
                                     <td><a href="view_buyer.php?up_id=<?php echo $row['us_id']; ?>" class="btn btn-primary btn-circle">&nbsp Activate  &nbsp</a><br><br>

                                   <a href="view_buyer.php?del_id=<?php echo $row['us_id']; ?>" class="btn btn-danger btn-circle">Deactivate</a></td>
                                    </tr>
...
if(isset($_REQUEST['del_id'])){
    ...
    $res= mysqli_query($con,$qry);
     $row=mysqli_fetch_array($res);
        ...
        $mail_body = "
        <p>Hi ".$row['uname'].",</p>
            <p>Your Buyer account has been deactivated by Area admin.</p>
            <p>Please contact Area admin for activating it.</p>
        <p>Please Open this link to login through your verified account- ".$base_url."login.php<p>Best Regards,<br />Team@E-NEIGHBORHOOD</p>         ";
           ...
           $mail1->SetFrom('eneighborhood14@gmail.com', "TEAM@E-NEIGHBORHOOD");
            ...
            if($mail1->Send())
                    {
                echo "<script>alert(' Account deactivated and Mail Successfully Sent!!');</script>";
                ?><script>window.location="view_buyer.php";</script><?php
                    }
            else
            {
...
if(isset($_REQUEST['up_id'])){
    ...
    $res= mysqli_query($con,$qry);
     $row=mysqli_fetch_array($res);
        ...
        $mail_body = "
       <p>Hi ".$row['uname'].",</p>
            <p>Your Buyer account has been activated by Area admin.</p>
        <p>Please Open this link to login through your verified account- ".$base_url."login.php<p>Best Regards,<br />Team@E-NEIGHBORHOOD</p>    ";
            $mail1 = new PHPMailer\PHPMailer\PHPMailer();
            $mail1->IsSMTP();
            $mail1->SMTPDebug  = false;
